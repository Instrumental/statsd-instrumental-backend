...
var key, host, port, timeout, flushInterval, secure, verifyCert, caChain, recordCounterRates, metricPrefix, metricFilters;
var formatMetricName = function(metric){ return metric; }
var filterMetric = function(metric){ return true; }
...
function build_payload(metrics, time_stamp) {
  ...
  payload = payload.filter(filterMetric);
...
exports.init = function instrumental_init(startup_time, config, events) {
  ...
  if(config.instrumental) {
    ...
    if(typeof(config.instrumental.metricFilters) === 'undefined'){
      metricFilters = [];
    } else {
      metricFilters = config.instrumental.metricFilters;
      filterMetric = function(metric) {
        metricName = metric.split(" ")[1];
        passedFilter = true;
        metricFilters.forEach(function(filter) {
          passedFilter = metricName.match(filter);
        });
        return passedFilter;
      }
    }    
