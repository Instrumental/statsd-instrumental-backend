...
var knownCerts  = ["instrumental.2018-08-19", "instrumental"];
...
function selectTlsOptionsVariation() {
  ...
  var successfulVariation = tlsOptionsVariations.find(function(variation){
  ...
  if (successfulVariation) { return successfulVariation; };
...
function initTlsOptionsVariations(config) {
  ...
  if(typeof(config.instrumental.caCertFile) === 'undefined'){
    // Node default certs
    ...
    log("Adding user supplied caCertFile option: " + config.instrumental.caCertFile);
    var certs = certsFromPaths([config.instrumental.caCertFile]);
...
exports.init = function instrumental_init(startup_time, config, events) {
  ...
  if(config.instrumental) {
    ...
    if(secure && verifyCert){
      ...
      // tlsOptionsVariations = tlsOptionsVariations || getTlsOptionsVariations(config);
