...
function instrumental_connection(host, port, onConnectCb){
  var connection;
  if (secure){
    connection = tls.connect({ port: port, host: host, rejectUnauthorized: verifyCert, ca: caChain }, function(){
    ...
    connection = net.createConnection({ port: port, host: host}, onConnectCB);
  }
...
exports.init = function instrumental_init(startup_time, config, events) {
  ...
  if(config.instrumental) {
    ...
    if(secure && verifyCert){
      ...
      caChain = [];
      for(var i = 0; i < knownCerts.length; i ++){
        var certPath = path.join(certDir, knownCerts[i] + ".ca.pem");
