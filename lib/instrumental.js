...
var key, host, port, timeout, flushInterval, secure, verifyCert, caChain, recordCounterRates, metricPrefix, metricFiltersInclude, metricFiltersExclude;
  ...
  if(config.instrumental) {
    ...
    if(secure && verifyCert){
      ...
      if(typeof(config.instrumental.caCertFile) === 'undefined'){
        certPaths = knownValidCertPaths();
      } else {
        log("caCertFile supplied, loading: " + config.instrumental.caCertFile);
        certPaths = [config.instrumental.caCertFile];
      }
      certPaths.forEach(function(certPath){
