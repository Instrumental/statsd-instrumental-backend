 ...
 *    host : Instrumental server address (default: "instrumentalapp.com")
...
function build_payload(metrics, time_stamp) {
  ...
  for(k in metrics.counters) {
    ...
    if(v !== 0) {
      payload.push("increment " + k + " " + v + " " + time_stamp);
    }
  }

  // gauge item.time 7292 121820381
  for(k in metrics.timers) {
    // Skip any timers that are currently empty (no new data).
    if(metrics.timers[k].length === 0) {
      continue;
    }

    for(v in metrics.timers[k]) {
      payload.push("gauge " + k + " " + metrics.timers[k][v] + " " + time_stamp);
    }
  }

  // gauge item.gauge 7292 121820381
  for(k in metrics.gauges) {
    payload.push("gauge " + k + " " + metrics.gauges[k] + " " + time_stamp);
  }
...
exports.init = function instrumental_init(startup_time, config, events) {
  ...
  if(config.instrumental) {
    ...
    host = config.instrumental.host || "instrumentalapp.com";
